...
async function init() {
  ...
  const [usuariosRegistrados, pedidosRegistrados, notificaciones, config] =
    await Promise.all([
      getUsuarios().catch((err) => {
        console.error("No se pudieron cargar los usuarios:", err);
        return [usuarioActivo]; // Usar solo el usuario actual
      }),
      esAdmin
        ? Promise.resolve([])
        : cargarPedidosUsuarioSeguros(usuarioActivo.idUsuario), // Admin no necesita pedidos
      getNotificacionesPorUsuario(usuarioActivo.idUsuario).catch((err) => {
      ...
      getConfiguracion().catch((err) => {
        ...
        return { horarioLimite: "10:30", feriados: [] };
      }),
    ]);
  ...
  const datosUsuario =
    usuariosRegistrados.find((u) => u.idUsuario === usuarioActivo.idUsuario) ||
    usuarioActivo;
  ...
  if (esAdmin) {
    ...
    document
      .getElementById("carta-asistencia")
    ...
    document
      .getElementById("carta-ultimo-pedido")
      ?.addEventListener("click", () => {
  ...
  if (recordatorios.length > 0) {
    ...
    setTexto(
      "home_aviso",
      ultimoRecordatorio.mensaje || "Tienes recordatorios pendientes",
    );
  } else {
