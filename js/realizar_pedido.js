import { requireAuth, getUsuarios, getPlatos, getConfig, getPedidos, getUsuarioActivo } from "./servicio.js";
...
function existePedidoHoy(pedidosFusionados, usuarioId, fechaISO) {
  return pedidosFusionados.some(p => p.usuario_id === usuarioId && p.fecha === fechaISO);
}
...
/* ============== Render UI (usa tus clases del HTML) ============== */
function renderPlatos(platosDelDia, contenedorCartas) {
  ...
  platosDelDia.forEach((plato, idx) => {
    ...
    carta.innerHTML = `
      ...
      <input type="radio" name="menu" class="realizar_pedido_container_cartas_carta_input" value="${plato.name}" ${idx===0?'checked':''}/>
    `;
...
async function init() {
  ...
  const [usuarios, platos, config, pedidosJSON] = await Promise.all([
    getUsuarios(), getPlatos(), getConfig(), getPedidos()
  ]);
  const datosUsuario = usuarios.find(u => u.id === usuarioActivo.id) || usuarioActivo;

  // 3) Contexto de hoy
  const fechaHoyISO = obtenerFechaActualISO();
  const nombreDiaHoy = normalizarDia(new Date().toLocaleDateString('es-AR', { weekday:'long' })); // ej. "miercoles"
  const esFeriadoHoy = esFeriado(config, fechaHoyISO);
  const dentroDeHorario = antesDeHorarioLimite(config.horario_limite_pedidos);
  const diasAsistencia = (datosUsuario.dias_asistencia || []).map(normalizarDia);
  const asisteHoy = diasAsistencia.includes(nombreDiaHoy);

  // 4) Habilitar botones de días según asistencia
  const botonesDias = Array.from(document.querySelectorAll('.realizar_pedido_container_dias_dia'));
  ...
  const contenedorCartas = document.querySelector('.realizar_pedido_container_cartas');

  // 6) Función para mostrar platos de un día
  function mostrarDia(diaNormalizado) {
    const platosDelDia = platos.filter(p => normalizarDia(p.day) === diaNormalizado);
  ...
  // 9) “Realizar pedido”
  const botonConfirmar = document.querySelector('.realizar_pedido_container_boton');
  botonConfirmar.addEventListener('click', () => {
    // El día actualmente mostrado es el último botón presionado; lo inferimos por el radio renderizado
    const radios = document.querySelectorAll('.realizar_pedido_container_cartas_carta_input');
    if (!radios.length) {
    ...
    const diaMostrado = document.querySelector('.realizar_pedido_container_dias_dia:focus')?.dataset.dia
      || nombreDiaHoy; // fallback: si no quedó focus, tomamos hoy
    ...
    if (!dentroDeHorario) {
      alert(`Cerrado. El horario límite (${config.horario_limite_pedidos}) ya pasó.`);
      return;
    }
